...
def download_chapter_th(event: threading.Event, chapter, manga_id, th_id):
	...
	for file in api.download_chapter(chapter, manga_id, pages):
		...
		download_steps[task_id] = {
			...
			"cover": cover,
			"name": "downloading",
			"percent": int(os.path.splitext(os.path.split(file)[1])[0]) / len(pages) / 2,
			"out": ""
		}
...
def download_chapters_th(event: threading.Event, chapters, manga_id, th_id):
	global download_steps
	info = api.get_info(manga_id)
	cover = info["cover"]
	manga_id = info["id"]
	...
	for chapter in chapters:
		...
		download_steps[task_id] = {
			...
			"percent": int(chapter) / len(chapters) / 2,
			"out": ""
		}
		pprint(download_steps[task_id])
		# eel.diplay_inividual_chapter_progresion(
		# 	download_steps[task_id]
		# )
	pprint(bookmarks)
...
def download_group(chapters, manga):
	...
	e = threading.Event()
	eid = id(e)
	th = threading.Thread(name=f"download_chapters_{manga}_{chapters[0]}_to_{chapters[-1:]}",
...
def download_chapter(chapter, manga_id):
	...
	e = threading.Event()
	eid = id(e)
	th = threading.Thread(name=f"download_chapter_{chapter}_{manga_id}",
...
download_group(["60", "61"], "jujutsu-kaisen")
